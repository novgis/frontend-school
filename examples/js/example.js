...
function createSubGalleryItem(config, name) {
  ...
  for (var key in config) {
    ...
    var content = $(
      "<div class='box box-default color-palette-box' id='" +
        name +
        "-" +
        key +
        "'></div>"
...
function createGalleryItemTitle(id, title) {
  ...
  return $(
    "<h3 class='category-title' id='title_" +
      id +
      "'>" +
      "<i class='fa " +
      menuItemIcon +
...
function createSubGalleryItemTitle(id, title) {
  return $(
    "<div class='box-header'>" +
      "<h3 class='box-title' id='category-type-" +
      id +
      "'>" +
      "&nbsp;&nbsp;&nbsp;&nbsp;" +
      title +
...
function createGalleryChart(example) {
  var target = "editor.html",
    ...
    thumbnail = example.thumbnail
      ? thumbLocation + "/img/" + example.thumbnail
      : "";

  var chartDiv = $(
    "<div class='col-xlg-2 col-lg-3 col-md-4 col-sm-6 col-xs-12'></div>"
  );
  var chart = $("<div class='chart'></div>");
  var link = $(
    "<a class='chart-link' target='_blank' href='" +
      target +
      "#" +
      href +
      "'></a>"
  ...
  var thumb = $(
    "<img class='chart-area' src='" + thumbnail + "' style='display: inline'>"
  );
  ...
  child
    ...
    .click(function (evt) {
      if (
        ...
        $(this).siblings("ul").children("li").hasClass("active")
      ) {
